...
class BilinearThermalMesher(GridMesher):
  """Bilinear structured grid mesher for thermal conduction."""
  ...
  def compute_connectivity_info(self)-> Tuple[np.ndarray, np.ndarray,
    ...
    nodenrs = np.reshape(np.arange(0, self.num_dofs), 
                        (1+self.nelx, 1+self.nely)).T
    elem_dof_vec = np.reshape(nodenrs[0:-1,0:-1]+1,
                        self.nelx*self.nely, 'F')
    elem_dof_mtrx = (np.matlib.repmat(elem_dof_vec, 4, 1).T +
      np.matlib.repmat(np.array([0, self.nely+1, self.nely, -1]),
      self.num_elems, 1))

    matrx_size = self.num_elems*self.dofs_per_elem**2
    iK = tuple(np.kron(elem_dof_mtrx, np.ones((self.dofs_per_elem, 1),dtype=int)).T
                .reshape(matrx_size, order ='F'))
    jK = tuple(np.kron(elem_dof_mtrx, np.ones((1, self.dofs_per_elem),dtype=int)).T
                .reshape(matrx_size, order ='F'))

    return elem_node, elem_dof_mtrx, iK, jK